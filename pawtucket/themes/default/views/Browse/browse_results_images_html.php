 ...
 */
	...
	$qr_res 			= $this->getVar('result',array('sort' => array('ca_objects.lastModified')));				// browse results (subclass of SearchResult)
...
//    $vs_current_sort = "Edited";
//	$vs_current_sort = = 'ca_objects.lastModified.timestamp';
...
$vs_default_placeholder ="";
//	$vs_default_placeholder_tag = "<div class='bResultItemImgPlaceholder'>".$vs_default_placeholder."</div>";
$vs_default_placeholder_tag ="";		
		...
		if ($vn_start < $qr_res->numHits()) {
			...
			while($qr_res->nextHit() && ($vn_c < $vn_hits_per_block)) {
				...
				$vs_thumbnail = "";
				$vs_type_placeholder = "";
				$vs_creator = $qr_res->get('ca_entities.preferred_labels.displayname',array('restrictToRelationshipTypes' => "creator",));
				$vs_subjects = $qr_res->get('ca_list_items.preferred_labels.name_plural');
$vs_moddate = $qr_res->get('ca_objects.lastModified');
$vs_lastmod = $qr_res->get("lastModified");
$vs_last_change = $t_list_item->getLastChangeTimestamp();
			$vs_last_change = "++ ".$vs_last_change." -- ".$vn_id;
//			$vs_last_change = $t_list_item->getLastChangeTimestamp();
//			$vs_last_change = $vs_last_change[timestamp] ;
				$vs_typecode = "";
				if ($vs_table == 'ca_objects') {
...
//							$vs_thumbnail = $vs_default_placeholder;
					...
					if ($qr_res->get('ca_objects.date')) {
						$vs_date = "<p>".$qr_res->get('ca_objects.date',array('convertCodesToDisplayText' => true,))."</p>";
					}
...
//						$vs_thumbnail = $vs_default_placeholder_tag;
					...
					$vs_image_detail_link	= caDetailLink($this->request, $vs_thumbnail, '', $vs_table, $vn_id);			
				}
				$vs_add_to_set_link = "";
	...
	<div class='bResultItemCol col-xs-{$vn_col_span_xs} col-sm-{$vn_col_span_sm} col-md-{$vn_col_span}'>
		<div class='bResultItem' onmouseover='jQuery(\"#bResultItemExpandedInfo{$vn_id}\").show();'  onmouseout='jQuery(\"#bResultItemExpandedInfo{$vn_id}\").hide();'>
	 	...
	 	<small>{$vs_idno_detail_link}</small> <br/>{$vs_thumbnail}<br />{$vs_label_detail_link} {$vs_creator}<br \>{$vs_date} 
